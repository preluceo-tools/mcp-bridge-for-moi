...
- **One Ctrl+Z undoes one agent step.** However much the agent does in a single call, one undo
...
```
  agent  ──stdio──▶  server  ◀──WebSocket──  bridge  ──▶  MoI's own API
                     (Node)                  (inside MoI)
```
...
If you have MoI for macOS, please open an issue with what you find — either way, including "it
doesn't work".
